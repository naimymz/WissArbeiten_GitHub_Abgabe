...
cramers_V <- function(x,y){  #Cramers V als Index für Korrelatin
...
#Chi_squared/ (Anzahl Beobachtungen * Minimum Zeilen und Spalten in Tabellendarstellung - 1)
...
r_pbis <- function(x,y){ #Punkt biseriale Korrelation, nimmt Werte zwischen -1 und 1 an
  ...
  yx_1 <- mean(y[x == 1]) #Mittelwert von y wo y = 1 ist 
  ...
  # -1 starker neg. Zusammenhang, 0 keinen, 1 starker pos. Zusammenhanger 
}
